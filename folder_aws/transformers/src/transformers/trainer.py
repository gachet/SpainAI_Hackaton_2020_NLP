...
class Trainer:
    ...
    def predict(
        self, test_dataset: Dataset, ignore_keys: Optional[List[str]] = None, metric_key_prefix: str = "eval", seq=True
    ) -> PredictionOutput:
        """
    ...
    def prediction_loop(
        ...
        for step, inputs in enumerate(dataloader):
            if not seq:
                ...
                loss, logits, labels = self.prediction_step_seq(
                    model, inputs, prediction_loss_only, ignore_keys=ignore_keys
                )
                logits = logits.reshape(logits.shape[0] // 30, 30, logits.shape[1])
            if loss is not None:
